...
$meta_boxes = array();
$meta_boxes = apply_filters ( 'horseclub_cmb_meta_boxes' , $meta_boxes );
foreach ( $meta_boxes as $meta_box ) {
	$my_box = new horseclub_cmb_Meta_Box( $meta_box );
}
 ...
 */
class horseclub_cmb_Meta_Box_Validate {
...
if (strtoupper(substr(PHP_OS, 0, 3)) === 'WIN') {
       //winblows
    define( 'HORSECLUB_CMB_META_BOX_URL', trailingslashit( str_replace( DIRECTORY_SEPARATOR, '/', str_replace( str_replace( '/', DIRECTORY_SEPARATOR, WP_CONTENT_DIR ), WP_CONTENT_URL, dirname(__FILE__) ) ) ) );

} else {
    define( 'HORSECLUB_CMB_META_BOX_URL', apply_filters( 'horseclub_cmb_meta_box_url', trailingslashit( str_replace( WP_CONTENT_DIR, WP_CONTENT_URL, dirname( __FILE__ ) ) ) ) );
}

/**
 * Create meta boxes
 */
class horseclub_cmb_Meta_Box {
	protected $_meta_box;
	...
	function __construct( $meta_box ) {
		...
		add_filter( 'horseclub_cmb_show_on', array( &$this, 'add_for_id' ), 10, 2 );
		add_filter( 'horseclub_cmb_show_on', array( &$this, 'add_for_page_template' ), 10, 2 );
	}
	...
	function add() {
		...
		foreach ( $this->_meta_box['pages'] as $page ) {
			if( apply_filters( 'horseclub_cmb_show_on', true, $this->_meta_box ) )
	...
	function save( $post_id)  {
		...
		foreach ( $this->_meta_box['fields'] as $field ) {
			...
			if ( isset( $field['validate_func']) ) {
				$ok = call_user_func( array( 'horseclub_cmb_Meta_Box_Validate', $field['validate_func']), $new );
 ...
 */
function horseclub_cmb_scripts( $hook ) {
	global $wp_version;
	...
	if ( $hook == 'post.php' || $hook == 'post-new.php' || $hook == 'page-new.php' || $hook == 'page.php' ) {
		...
		wp_register_script( 'horseclub-cmb-timepicker', HORSECLUB_CMB_META_BOX_URL . 'js/jquery.timePicker.min.js' );
		wp_register_script( 'horseclub-cmb-scripts', HORSECLUB_CMB_META_BOX_URL . 'js/cmb.js', $cmb_script_array, '0.9.1' );
		wp_localize_script( 'horseclub-cmb-scripts', 'cmb_ajax_data', array( 'ajax_nonce' => wp_create_nonce( 'ajax_nonce' ), 'post_id' => get_the_ID() ) );
		wp_enqueue_script( 'horseclub-cmb-timepicker' );
		wp_enqueue_script( 'horseclub-cmb-scripts' );
		wp_register_style( 'horseclub-cmb-styles', HORSECLUB_CMB_META_BOX_URL . 'style.css', $cmb_style_array );
		wp_enqueue_style( 'horseclub-cmb-styles' );
	}
}
add_action( 'admin_enqueue_scripts', 'horseclub_cmb_scripts', 10 );

function horseclub_cmb_editor_footer_scripts() { ?>
	<?php
	if ( isset( $_GET['horseclub_cmb_force_send'] ) && 'true' == $_GET['horseclub_cmb_force_send'] ) {
...
add_action( 'admin_print_footer_scripts', 'horseclub_cmb_editor_footer_scripts', 99 );

// Force 'Insert into Post' button from Media Library
add_filter( 'get_media_item_args', 'horseclub_cmb_force_send' );
function horseclub_cmb_force_send( $args ) {
	...
	if ( isset( $_POST['attachment_id'] ) && '' != $_POST["attachment_id"] ) {
		...
		echo '
			<script type="text/javascript">
				...
				jQuery(function($) {
					if (cmbGetParameterByNameInline("horseclub_cmb_force_send")=="true") {
...
add_action( 'wp_ajax_cmb_oembed_handler', 'horseclub_cmb_oembed_ajax_results' );
/**
 * Handles our oEmbed ajax request
 */
function horseclub_cmb_oembed_ajax_results() {
	...
	if ( empty( $oembed_string ) ) {
		$return = '<p class="ui-state-error-text">'. esc_html__( 'Please Try Again', 'horseclub' ) .'</p>';
		$found = 'not found';
	} else {
		...
		if ( $check_embed && $check_embed != $fallback ) {
			// Embed data
			$return = '<div class="embed_status">'. $check_embed .'<a href="#" class="cmb_remove_file_button" rel="'. $_REQUEST['field_id'] .'">'. esc_html__( 'Remove Embed', 'horseclub' ) .'</a></div>';
			// set our response id
			$found = 'found';

		} else {
			// error info when no oEmbeds were found
			$return = '<p class="ui-state-error-text">'.sprintf( esc_html__( 'No oEmbed Results Found for %s. View more info at', 'horseclub' ), $fallback ) .' <a href="http://codex.wordpress.org/Embeds" target="_blank">codex.wordpress.org/Embeds</a>.</p>';
			// set our response id
			$found = 'not found';
		}
	}
