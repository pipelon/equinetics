 ...
 */
$title = $source = $image = $custom_src = $onclick = $img_size = $external_img_size = $a_class =
...
switch ( $onclick ) {
	case 'img_link_large':
$a_class = ' class="image-popup-no-margins"';
		if ( 'external_link' === $source ) {
			$link = $custom_src;
...
if ( $link ) {
	...
	$html = '<a  ' . vc_stringify_attributes( $a_attrs ) . ' '. $a_class . '>' . $html . '</a>';
}
